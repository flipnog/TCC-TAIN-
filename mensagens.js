...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    const me = usnap.data();
    if (me.tipo && me.tipo !== "estudante") {
      alert("Esta área de mensagens é para Estudantes.");
      window.location.href = "login.html";
      return;
    }
    ME = { uid:user.uid, ...me };
    meInfo.textContent = `${me.nome || user.displayName || "Estudante"} — ${me.email || user.email || ""}`;
...
async function loadThreads(){
  ...
  try{
    // Sem orderBy (evita exigir índice); ordenamos em memória
    const q1 = query(
      collection(db,"matches"),
      where("estudanteId","==", ME.uid),
      limit(100)
    );
    const snap = await getDocs(q1);

    if (snap.empty){
      threadsEl.innerHTML = `<div class="thread-item muted">Você ainda não possui conversas com tutoras.</div>`;
      return;
    }
    ...
    for (const it of items){
      const tutorSnap = await getDoc(doc(db,"usuarios", it.meta.tutoraId));
      const tutor = tutorSnap.exists() ? tutorSnap.data() : { nome:"Tutora", email:"" };

      const item = document.createElement("div");
      item.className = "thread-item";
      item.dataset.tid = it.tid;
      item.innerHTML = `
        <div><strong>${tutor.nome || "Tutora"}</strong></div>
        <div class="muted" style="font-size:.9rem">${tutor.email || ""}</div>
      `;
      item.addEventListener("click", ()=> openThread(it.tid, tutor));
...
async function openThread(tid, tutorDoc){
  ACTIVE = { tid, tutorDoc };
  highlightActive(tid);

  chatWith.textContent = tutorDoc?.nome || "Tutora";
  chatWithEmail.textContent = tutorDoc?.email || "";
...
btnSend.addEventListener("click", async ()=>{
  ...
  try{
    ...
    msgText.value = "";
    autoresize(); // mantém o textarea ajustado
  }catch(e){
